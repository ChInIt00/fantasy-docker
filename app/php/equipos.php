<?php
include '../header.php'; // Header al mismo nivel de index.php
...
// Obtenemos jugadores de este equipo, no NULL y con valor más reciente
$sql_players = "
SELECT p.id, p.name
FROM players p
...
";
$stmt3 = $mysqli->prepare($sql_players);
$stmt3->bind_param('i', $team_id);
$stmt3->execute();
$res_players = $stmt3->get_result();
...
<head>
    <meta charset="utf-8">
    <title>Equipo: <?php echo htmlspecialchars($team_name); ?></title>
...
<main>
    <div class="team-header">
        ...
        <div>
            ...
            <p style="margin:4px 0;color:#666">Plantilla oficial</p>
        </div>
    </div>

    <div class="tabs">
        <a href="/php/equipos.php?team=<?php echo urlencode($team_slug); ?>" class="active">Plantilla</a>
        <a href="/php/partidos.php?team=<?php echo urlencode($team_slug); ?>">Calendario</a>
        <a href="/php/once.php?team=<?php echo urlencode($team_slug); ?>">Once probable</a>
    </div>
                ...
                <tbody>
                    <?php foreach ($players as $pl): ?>
                        <tr>
                            ...
                            <td><a href="/php/player.php?id=<?php echo (int)$pl['id']; ?>">Ver</a></td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
